...
fig.def()
par(mar=c(0,0,3,0))
pie(conteggi, main = "Grafico a Torta 2D", col = c(iblue, lighten(iblue,.75), lighten(iblue,.5), lighten(iblue,.25)))
```
...
```{r,fig.cap="Attenzione alla terza dimensione",fig.align='center'}
# Grafico a torta normale
fig.def()
# Grafico a torta 3D
clip(-1.1,1.1,-.5,.5)
...
par(mar = c(0, 0, 0, 0))
pie3D(conteggi, 
      col = c(iblue, lighten(iblue,.75), lighten(iblue,.5), lighten(iblue,.25)), 
      explode = 0.0, labelcex = 0.8,theta = .34,start = 3/4*pi,height = .1)
invisible(dev.off())
...
img_with_margin <- image_border(img_trimmed, color = "white", geometry = "50x50")
