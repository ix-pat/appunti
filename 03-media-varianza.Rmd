...
5. Associatività. Sia una popolazione, $\mathcal{P}$, formata da $K$
gruppi con medie e numerosità: ($\bar{x}_{1};\ n_{1}$),
($\bar{x}_{2};\ n_{2}$), $\ldots$, ($\bar{x}_{K};\ n_{K}$). Allora,
la media totale $\bar{x}_{T}$ di $\mathcal{P}=$ è data da

\[
  \bar{x}_{T}
= \frac{\mbox{Tot}{ \{\mathcal{P}}_1\} +  \cdots +
        \mbox{Tot}{ \{\mathcal{P}}_K\}} {n_{1} + \cdots + n_{K}}
