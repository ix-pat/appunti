...
differenza tra $\hat\mu_\text{obs}$ e $\mu_0$ **non è significativa**. L'ipotesi
...
Z &>& z_\alpha \\
Z &>& `r qnorm(.95)` \\
   \frac{\hat\mu - \mu_{0}} {\sigma/\sqrt{n}}&>&`r qnorm(.95)` \\
   \hat\mu - \mu_{0} &>&`r qnorm(.95)`\frac\sigma{\sqrt{n}} \\
   \hat\mu &>& \mu_{0} + `r qnorm(.95)`\frac\sigma{\sqrt{n}} \\   
   &>&   30 + `r qnorm(.95)`\frac 5{ \sqrt{ 25 }}\\
   &>&   `r 30+qnorm(.95)`
   \end{eqnarray*}
Se $H_0$ fosse vera $\mu=30$, 5 volte su 100, un campione di ampiezza $n=25$
ha una media che supera $`r 30+qnorm(.95)`$.
:::
...
Z &>& z_\alpha \\
Z &>& `r qnorm(.99)` \\
   \frac{\hat\mu - \mu_{0}} {\sigma/\sqrt{n}}&>&`r qnorm(.99)` \\
   \hat\mu - \mu_{0} &>&`r qnorm(.99)`\frac\sigma{\sqrt{n}} \\
   \hat\mu &>& \mu_{0} + `r qnorm(.99)`\frac\sigma{\sqrt{n}} \\   
   &>&   30 + `r qnorm(.99)`\frac 5{ \sqrt{ 25 }}\\
   &>&   `r 30+qnorm(.99)`
   \end{eqnarray*}
Se $H_0$ fosse vera $\mu=30$, 5 volte su 100, un campione di ampiezza $n=25$
ha una media che supera $`r 30+qnorm(.99)`$.
:::
...
h1 <- ">"
um <- "g"
cat(ztest_mu(muh = muh,s = s,n = n,mu0 = mu0,h1 = h1,pv_only = T,um = um,pvalue = F))
```
